...
from app.security import retrieve_credit_card_info, store_credit_card_info
...
async def test_retrieve_credit_card_info(db_session_test, get_credit_card_id):
    credit_card = await retrieve_credit_card_info(db_session_test, get_credit_card_id)
    assert credit_card["card_number"] == "1234567812345678"
    assert credit_card["card_holder_name"] == "John Doe"
    assert credit_card["expiration_date"] == "12/23"

    query = select(CreditCard).where(CreditCard.id == get_credit_card_id)
