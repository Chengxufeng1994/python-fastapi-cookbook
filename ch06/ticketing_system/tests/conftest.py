import pytest
from app.database import Base, Event, Sponsor, Sponsorship, Ticket, TicketDetails
...
async def db_session_test(
    ...
    TestingAsynSessionLocal = sessionmaker(bind=db_engine_test, class_=AsyncSession)
...
async def fill_database_with_tickets(db_session_test):
    tickets = [Ticket(show="The Rolling Stones Concert") for _ in range(10)]
...
@pytest.fixture
async def add_event_and_sponsor_and_sponsorship(db_session_test, add_event_and_sponsor):
    sponsorship = Sponsorship(event_id=1, sponsor_id=1, amount=10)
    async with db_session_test.begin():
