...
def get_premium_user(
    current_user: Annotated[UserCreateRequestWithRole, Depends(get_current_user)],
):
...
def all_user_can_access(
    user: Annotated[UserCreateRequestWithRole, Depends(get_current_user)],
):
...
def only_premium_user_can_access(
    user: Annotated[UserCreateRequestWithRole, Depends(get_premium_user)],
):
