...
from .operations import get_user_by_username, update_user
...
def generate_totp_uri(secret, user_email):
    return pyotp.totp.TOTP(secret).provisioning_uri(name=user_email, issuer_name="YourAppName")
...
def enable_mfa(
    ...
    db_user = update_user(session, db_user)
